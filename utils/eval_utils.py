import sys
COCO_PATH = '/data/coco-caption_python3/' # i.e. /home/user/code/coco-caption
...
def score_generation(anno_file, result_file, use_spice=False, flag=False):
    coco = COCO(anno_file)
    ...
    if use_spice:
        ...
        coco_eval.evaluate_no_spice()

    if not flag:
        return copy.deepcopy(coco_eval.eval)
    else:
        return copy.deepcopy(coco_eval.eval), coco_eval.evalImgs

def score_generation_by_type(anno_file, result_file, type_file, use_spice=False):
    coco = COCO(anno_file)
    coco_res = coco.loadRes(result_file)
    coco_eval = COCOEvalCap(coco, coco_res)
